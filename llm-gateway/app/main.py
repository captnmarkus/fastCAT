...
def _extract_target_langs(text: str) -> list[str]:
  match = re.search(r"\bto\s+([a-z]{2,3}(?:-[a-z0-9]{2,8})?(?:\s*,\s*[a-z]{2,3}(?:-[a-z0-9]{2,8})?)*)", text, re.IGNORECASE)
  if not match:
    return []
...
def _chunk_text(text: str, chunk_size: int = 28) -> list[str]:
  ...
  while idx < len(normalized):
    out.append(normalized[idx : idx + chunk_size])
    idx += chunk_size
  return out
  ...
  async def generate_events() -> Any:
    ...
    if not config.enabled:
      ...
      if plan.get("mode") == "project_wizard":
        resolved_plan = await _resolve_project_wizard_plan(
          request,
          trace_id=trace_id,
          user_context=user_context,
          history=history,
