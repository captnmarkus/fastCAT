...
import { registerProjectRoutesPart6RenderedPreviewRoutes } from "./projects.routes.part6.rendered-preview.routes.js";

export type RenderedPreviewRouteHelpers = {
  computeDraftRevisionId: (params: { projectId: number; fileId: number; taskId: number | null }) => Promise<string>;
  findRenderedPreviewArtifactByCacheKey: (params: { projectId: number; fileId: number; cacheKey: string }) => Promise<{ id: number } | null>;
  pruneRenderedPreviewJobs: () => void;
  renderedPreviewJobsByCacheKey: Map<string, string>;
  renderedPreviewJobsById: Map<string, any>;
  resolveRenderedPreviewSelection: (params: {
    projectId: number;
    fileId: number | null;
    taskId: number | null;
    targetLang: string | null;
  }) => Promise<any>;
  runRenderedPreviewGenerationJob: (params: {
    jobId: string;
    cacheKey: string;
    projectId: number;
    fileId: number;
...
export async function registerProjectRoutesPart6RenderedPreview(app: FastifyInstance) {
...
<html>
  ...
  await registerProjectRoutesPart6RenderedPreviewRoutes(app, {
